...
$(function () {
    ...
    $("#btn_delete").click(function (){
        ...
        if(temp.length<=0) {
            ...
            for(var i=0;i<temp.length;i++){
               /* putTemp[i]=temp[i].id;
                var aa = temp[i].id;*/
                bb =bb+ "ids="+temp[i].leaderId+"&";
            }
            bb = bb.substr(0,bb.length-1);
            console.log(bb);
            $.ajax({
                type: "DELETE",
                url: "/leader/deletes"+"?"+ bb,
               /* data: {"ids": bb},*/
                dataType: "json",
                success: function(data) {
                    if(data.code == "200"){
                        alert("删除成功");
                        window.location.href="manageLeader.html";
                    }
                    else{
                        alert("删除失败");
                        window.location.href="manageLeader.html";
    ...
    $("#btn_edit").click(function(){
       ...
       if(temp.length<=0){
           ...
           $("#leaderNameEdit").val(temp[0].leaderName);

           //提交
           $("#btn_submit_Edit").click(function () {
               var id = temp[0].leaderId;
               var leaderName = $("#leaderNameEdit").val();


               if(validateEdit(leaderName)){
                   $.ajax({
                       type: "put",
                       url: '/leader/update',
                       data: {
    ...
    $("#btn_submit").click(function (){
        ...
        if(validateAddForm(leaderName,password,repassword)){
            console.log("add");
            $.ajax({
                type: "post",
                url: '/leader/add',
...
/*添加数据校验*/
function validateAddForm(studentName,password,repassword) {
    if(studentName == "" ||  password == "" || repassword == ""){
        alert("请填写必要的信息");
        return false;
    }
...
/*修改校验*/
function validateEdit(studentName) {
    if(studentName == ""){
        alert("请填写必要的信息");
        return false;
    }
